...
void solve(){
    ...
    int mx_divisor=0,mx_prime_div=0;
    for(int i=1;i<=N;i++)
    {
        mx_divisor=max(mx_divisor,(int)divisor[i].size());
        mx_prime_div=max(mx_prime_div,(int)prime_div[i].size());
    }
    cout<<mx_divisor<<" "<<mx_prime_div<<"\n";
}
