...
// bitset<octroi> vc;
flt min_distance(ll val, vec &v){
    return *lower_bound(all(v), val);
}

void solve(){
    ll n, m, x, y; cin>>n>>m>>x>>y;
    vec bnk(n), kaif(m), trnsfr(m);
    ...
    rep(i, 0, m) cin>>trnsfr[i];

    ll mn_distance = LLONG_MAX;
    flt ttl = 0;
    rep(i, 0, m){
        flt cst1 = (trnsfr[i]*x)/1000*1.0;
        trnsfr[i] *= (trnsfr[i]<0)? (-1):1;
        flt cst2 = ((min_distance(trnsfr[i], bnk) * y)*60)/(100*1.0);

        flt mn = min(cst1, cst2);
