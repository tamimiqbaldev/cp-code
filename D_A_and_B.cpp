...
#define Caina_Meye_Tumi_Onno_Karo_How FAST_IO
...
ll min_swaps(const string &s, char ch) {
    VEC pos;
    ...
    ll k = size(pos);
    VEC shifted(k);
    for (ll i = 0; i < k; i++) shifted[i] = pos[i] - i;

    nth_element(all(shifted), shifted.begin() + k/2);
    ll median = shifted[k/2];

    ll cost = 0;
    for (ll i = 0; i < k; i++) cost += abs(shifted[i] - median);
    return cost;
}
...
void solve() {
    ...
    ll ans = min(min_swaps(s, 'a'), min_swaps(s, 'b'));
    cout << ans << nl;
}

signed main() {
    Caina_Meye_Tumi_Onno_Karo_How
    int t; cin >> t;
    while (t--) solve();
    return 0;
}