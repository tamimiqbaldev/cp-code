...
void solve(){
    ...
    rep(i, 0, n){
        int wrktime, ded; cin>>wrktime>>ded;
        mp.push_back({wrktime, ded});
    }

    sort(all(mp), [](const pair<int, int>& a, const pair<int, int>& b){
            if (a.first == b.first) return a.second > b.second; 
            return a.first < b.first; 
    });
    // sort(mp.begin(), mp.end(), [](pair<int, int>&pra, pair<int, int>&prb){
    //     if(pra.first==prb.first){
    //         return pra.second>prb.second;
    //     }else{
    //         pra.first<pra.second;
    //     }
    // });
